...
const AddCourse = ({ data, open, setOpen, getCourses }) => {
  ...
  const submit = () => {
    form
      ...
      .then(async (values) => {
        if (!imageFile) {
          toast.error("برجاء اختيار صورة للدورة");
          return;
        }
        ...
        try {
          ...
          // Add course with the uploaded image URL
          const response = await API.addCourse(values, uploadedImageUrl);
          ...
          if (response.data.status === "success") {
            ...
            toast.error(response.data.message || "فشل إضافة الدورة");
          }
        } catch (error) {
          console.error("Error adding course:", error);
    ...
    >
      <Form form={form} layout="vertical" className="h-full flex flex-col">
        ...
        <Form.Item
          name="image"
          label="صورة الدورة"
          rules={[{ required: true, message: "برجاء اختيار صورة للدورة" }]}
        >
          <Upload
            name="avatar"
